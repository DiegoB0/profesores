...
app.get('/profesores/listar', (req, res) => {
...
app.post('/profesores/modificar', (req, res) => {
	...
	sql =
		'UPDATE profesores set nombres=?, apellidos=?, fnacimiento=?, email=?, sexo=?, estadocivil=?, tcasa=?, curp=?, tcelular=?, calle=?, colonia=?, cp=?, municipio=?, estado=? WHERE clave=?';
	db.query(
		sql,
		[
			nombre,
