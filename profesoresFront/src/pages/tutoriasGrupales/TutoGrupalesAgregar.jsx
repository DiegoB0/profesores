...
const TutoGrupalesAgregar = () => {
	const { createTutoriaGrupal, getTutoriaGrupal, updateTutoriaGrupal } =
		useTutoriasGrupales();
	...
	useEffect(() => {
		const loadTutorias = async () => {
			if (params.id) {
				const tutoria = await getTutoriaGrupal(params.id);
				console.log(tutoria);
				setTutorias({
		...
		<div className="flex flex-col items-center justify-center mt-12">
			...
			<Formik
				...
				onSubmit={async (values) => {
					if (params.id) {
						await updateTutoriaGrupal(params.id, values);
						navigate('/tutorias/grupales');
						notifyUpdated();
					} else {
						await createTutoriaGrupal(values);
						navigate('/tutorias/grupales');
						notifyPosted();
					}
			...
			>
				{({ handleChange, handleSubmit, values, isSubmitting }) => (
					<Form
						...
						className="w-full max-w-lg mt-10 rounded-xl bg-white p-10 mb-10"
					>
...
export default TutoGrupalesAgregar;
